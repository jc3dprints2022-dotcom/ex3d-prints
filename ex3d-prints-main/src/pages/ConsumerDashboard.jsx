...
import AffiliateHubTab from "@/components/consumer/AffiliateHubTab";
...
export default function ConsumerDashboard() {
  ...
  useEffect(() => {
    ...
    if (review === 'true' && orderId) {
      // Will open review modal once orders are loaded
      setSelectedOrder({ id: orderId, _pendingReview: true });
    }
  }, []);
  ...
  const loadOrders = async (currentUser) => {
    try {
      ...
      const sorted = userOrders.sort((a, b) => new Date(b.created_date) - new Date(a.created_date));
      setOrders(sorted);
      // Handle pending review from URL param
      const urlParams = new URLSearchParams(window.location.search);
      const orderId = urlParams.get('orderId');
      const review = urlParams.get('review');
      if (review === 'true' && orderId) {
        const foundOrder = sorted.find(o => o.id === orderId);
        if (foundOrder) {
  ...
  const sidebarItems = [
    ...
    ...(user?.business_roles?.includes('maker') ? [{ id: "maker", label: "Maker Hub", icon: Printer }] : []),
    ...(user?.business_roles?.includes('designer') ? [{ id: "designer", label: "Designer Hub", icon: Paintbrush }] : []),
    ...(user?.business_roles?.includes('affiliate') ? [{ id: "affiliate", label: "Affiliate Hub", icon: Link2 }] : []),
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <main className="flex-1 space-y-6">
                  ...
                  <CardContent>
                    {orders.length === 0 ? (
                      <div className="text-center py-12">
                        ...
                        <p className="text-gray-500">No orders yet. <a href="/Marketplace" className="text-teal-600 underline">Shop now</a></p>
                      </div>
                    ) : (
                      <div className="space-y-3">
                        {orders.slice(0, showAllOrders ? orders.length : 10).map(order => {
                          ...
                          return (
                            <div
                              key={order.id}
                              className={`border rounded-lg overflow-hidden ${expandedOrderId === order.id ? 'border-teal-400 shadow-md' : 'border-gray-200'}`}
                            >
                              {/* Order header row - always visible */}
                              <button
                                className="w-full flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-4 text-left hover:bg-gray-50 transition-colors"
                                onClick={() => setExpandedOrderId(isExpanded ? null : order.id)}
                              >
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                  {order.items?.[0]?.images?.[0] && (
                                    <img src={order.items[0].images[0]} alt="" className="w-12 h-12 object-cover rounded-lg flex-shrink-0" />
                                  )}
                                  <div className="min-w-0">
                                    <p className="font-medium text-sm truncate">{order.items?.[0]?.product_name}{order.items?.length > 1 ? ` +${order.items.length - 1} more` : ''}</p>
                                    <p className="text-xs text-gray-500">#{order.id.slice(0,8)} · {new Date(order.created_date).toLocaleDateString()}</p>
                                  </div>
                                </div>
                                <div className="flex items-center gap-3 flex-shrink-0">
                                  {getOrderStatusBadge(order.status)}
                                  <span className="text-sm font-semibold text-gray-800">${order.total_amount?.toFixed(2)}</span>
                              ...
                              {isExpanded && (
                                <div className="border-t bg-gray-50 p-4 space-y-4">
                                    ...
                                    <div className="space-y-2">
                                      {(order.items || []).map((item, i) => (
                                        <div key={i} className="flex items-center gap-3 text-sm">
                                          {item.images?.[0] && <img src={item.images[0]} alt="" className="w-10 h-10 object-cover rounded" />}
                                          <div className="flex-1">
                                            <p className="font-medium">{item.product_name}</p>
                                            <p className="text-gray-500">Qty: {item.quantity} · {item.selected_material} · {item.selected_color}</p>
                                          </div>
                                  ...
                                  <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                                    ...
                                    <p><span className="font-medium">Placed:</span> {new Date(order.created_date).toLocaleDateString()}</p>
                                    {order.shipped_at && <p><span className="font-medium">Shipped:</span> {new Date(order.shipped_at).toLocaleDateString()}</p>}
                                    {order.delivered_at && <p><span className="font-medium">Delivered:</span> {new Date(order.delivered_at).toLocaleDateString()}</p>}
                                    {order.tracking_number && <p><span className="font-medium">Tracking:</span> {order.tracking_number}</p>}
                                  </div>
                                  ...
                                  <div className="flex flex-wrap gap-2 pt-2 border-t">
                                    ...
                                    {order.status === 'delivered' && (
                                      <Button size="sm" className="bg-teal-600 hover:bg-teal-700 text-white" onClick={() => { setSelectedOrder(order); setReviewModalOpen(true); }}>
                        ...
                        {orders.length > 10 && (
                          <Button variant="outline" onClick={() => setShowAllOrders(!showAllOrders)} className="w-full">
                            {showAllOrders ? 'Show Less' : `Show ${orders.length - 10} More Orders`}
                          </Button>
                        )}
                      </div>
                    )}
                  ...
                  <CardContent>
                    {customRequests.length === 0 ? (
                      <div className="text-center py-12">
                        ...
                        <Button onClick={() => window.location.href = '/CustomPrintRequest'}>Submit New Request</Button>
                      </div>
                    ) : (
                      <div className="space-y-4">
                          ...
                          <Card key={request.id} className="border-l-4 border-purple-500">
                            <CardContent className="p-4">
                              <div className="flex flex-col sm:flex-row gap-4">
                                {request.images?.[0] && <img src={request.images[0]} alt={request.title} className="w-20 h-20 object-cover rounded-lg" />}
                                <div className="flex-1">
                                  <div className="flex flex-col sm:flex-row justify-between gap-4">
                                    <div className="flex-1">
                                      ...
                                      <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                                        ...
                                        {request.quoted_price && <p className="font-semibold text-green-600">Quote: ${request.quoted_price.toFixed(2)}</p>}
                                      </div>
                                    ...
                                    {request.status === 'quoted' && (
                                      <div className="flex flex-col gap-2">
                                        <Button onClick={() => handleAcceptQuote(request)} className="bg-green-600 hover:bg-green-700">
                                          <ThumbsUp className="w-4 h-4 mr-2" />Accept & Add to Cart
                                        </Button>
                                        <Button variant="outline" onClick={() => handleDeclineQuote(request.id)}>Decline</Button>
                                      </div>
                                    )}
                                  </div>
                                </div>
                        ...
                        {customRequests.length > 5 && (
                          <Button variant="outline" onClick={() => setShowAllCustomRequests(!showAllCustomRequests)} className="w-full">
            ...
            {activeSection === "affiliate" && user?.business_roles?.includes('affiliate') && (
              <AffiliateHubTab user={user} onUpdate={loadDashboardData} />
            )}
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <div className="mt-8 pt-8 border-t border-red-200">
                    <h3 className="text-lg font-semibold text-red-600 mb-2 dark:text-red-400">Danger Zone</h3>
      ...
      {/* Account Deletion Dialog — 24-hour cool-down */}
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Schedule Account Deletion</AlertDialogTitle>
            <AlertDialogDescription>
              Your account will be permanently deleted in <strong>24 hours</strong>. You will receive a confirmation email. If you change your mind, you can cancel by logging back in within that window.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={async () => {
                ...
                try {
                  ...
                  const pendingKey = `delete_pending_${user?.id}`;
                  const pendingAt = localStorage.getItem(pendingKey);
                  if (pendingAt) {
                    const hoursLeft = Math.ceil((24 * 60 * 60 * 1000 - (Date.now() - parseInt(pendingAt))) / 3600000);
                    toast({ title: "Deletion already scheduled", description: `Your account will be deleted in ~${hoursLeft}h. Log in to cancel.`, variant: "destructive" });
                  } else {
                    // Record the pending deletion locally and notify backend
                    localStorage.setItem(pendingKey, Date.now().toString());
                    await base44.functions.invoke('deleteUserAccount', { cooldown: true }).catch(() => {});
                    toast({
                      title: "Deletion scheduled",
                      description: "Your account will be deleted in 24 hours. Check your email for a cancellation link.",
                      duration: 8000
                    });
                  }
                } catch (error) {
                  toast({ title: "Failed to schedule deletion", description: error.message, variant: "destructive" });
                }
