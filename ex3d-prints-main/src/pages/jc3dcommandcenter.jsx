...
import OrderManagementSection from "../components/commandcenter/OrderManagementSection";
...
export default function JC3DCommandCenter() {
  ...
  const [activeSection, setActiveSection] = useState(urlParams.get("section") || "dashboard");
  ...
  const renderSection = () => {
    switch (activeSection) {
      ...
      case "orders":      return <OrderManagementSection />;
      case "payments":    return <PaymentsFinancialsSection />;
      case "exp":         return <ExpManagementSection />;
      case "email":       return <EmailManagementSection />;
      case "makers":      return <MakerToolsSection />;
      case "items":       return <ItemManagementSection />;
      case "messages":    return <MessagesSection />;
      case "settings":    return <SystemSettingsSection />;
  ...
  return (
    <div className="min-h-screen bg-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Active Section Content — boundary keyed by section so one crashing
            section shows an inline error instead of white-screening the admin */}
        <div>
          <ErrorBoundary key={activeSection} compact title={`The "${activeSection}" section hit an error.`}>
            {renderSection()}
          </ErrorBoundary>
        </div>
      </div>
