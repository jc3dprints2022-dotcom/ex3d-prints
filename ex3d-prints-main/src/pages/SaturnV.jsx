...
const PAGE_SOURCE  = "saturnv_landing";
            ...
            <div key={i} className="flex-shrink-0" style={{ minWidth: "100%", height: "100%" }}>
              {isVideo(src) ? (
                ...
                <div className="w-full h-full cursor-zoom-in" onClick={() => onLightbox(src)}>
                  <img src={src} alt={`Saturn V photo ${i + 1}`} className="w-full h-full object-contain p-3" draggable={false} />
                </div>
              )}
            </div>
          ))}
...
export default function SaturnVLanding() {
  const [adding, setAdding]         = useState(null);
  const [lightbox, setLightbox]     = useState(null);
  const [bannerCopied, setBannerCopied] = useState(false);
  const [showSticky, setShowSticky] = useState(false);
  const [heroImage, setHeroImage]   = useState(FALLBACK_SATURN);
  const [carouselImages, setCarouselImages] = useState([]);
  const [slsBundleImg, setSlsBundleImg]     = useState(FALLBACK_SLS);
  ...
  useEffect(() => {
    // Capture affiliate ref from URL path (e.g. /SaturnV/AFFILIATE_ID) or ?ref= param
    ...
    base44.entities.Product.filter({ status: "active" })
      .then(products => {
        ...
        const saturnImgs = saturn?.images ?? [];
        setHeroImage(VERTICAL_SATURN);
        setCarouselImages(saturnImgs.filter(img => img !== VERTICAL_SATURN));
        setSlsBundleImg(VERTICAL_SLS);
      })
      .catch(() => { setHeroImage(FALLBACK_SATURN); setSlsBundleImg(FALLBACK_SLS); });
  }, []);
  ...
  const addToCart = async (type = "saturn") => {
    setAdding(type); trackEvent("add_to_cart", { type });
    try {
      const user = await base44.auth.me().catch(() => null);
      const items = [];
      if (type === "saturn") items.push({ product_id: SATURN_V_ID, product_name: "Saturn V", price: SATURN_PRICE, image: heroImage });
      if (type === "moon_collection") {
        items.push({ product_id: SATURN_V_ID, product_name: "Saturn V (Moon Missions Collection)", price: MOON_SATURN_PRICE, image: heroImage });
        items.push({ product_id: SLS_ID,      product_name: "SLS (Moon Missions Collection)",      price: MOON_SLS_PRICE,   image: slsBundleImg });
      }
      if (user) {
    ...
    <div className="min-h-screen bg-[#09080e] text-white overflow-x-hidden" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      {showSticky && (
        <div className="fixed bottom-0 left-0 right-0 z-50 p-3 sm:hidden"
          ...
          <button onClick={() => addToCart("saturn")} disabled={adding !== null}
            ...
            {adding === "saturn" ? "Adding..." : "Order Now · $39"}
          </button>
        </div>
      )}
      ...
      <section className="max-w-sm mx-auto px-5 pt-6 pb-8">
        ...
        <div className="w-full rounded-2xl overflow-hidden mb-5 cursor-zoom-in"
          ...
          {heroImage ? (
            <img src={heroImage} alt="Saturn V 3D printed model kit"
        ...
        <p className="text-center text-gray-400 text-sm mb-5 leading-relaxed">
          Detailed 22" snap-fit Saturn V model kit. Assembles in 20 minutes. Some glue needed.
        </p>
        ...
        <div ref={heroCTARef}>
          <button onClick={() => addToCart("saturn")} disabled={adding !== null}
            ...
            {adding === "saturn" ? "Adding..." : "Order Now"}
          </button>
        </div>
      ...
      <section className="border-t border-white/5 py-10 px-5">
        <div className="max-w-sm mx-auto">
          ...
          <div className="flex gap-3 mb-5 justify-center">
            {[
              { src: heroImage,    alt: "Saturn V",    label: 'Saturn V 22"',    border: "rgba(251,146,60,0.3)" },
              { src: slsBundleImg, alt: "SLS Artemis", label: 'SLS Artemis 20"', border: "rgba(96,165,250,0.3)"  },
