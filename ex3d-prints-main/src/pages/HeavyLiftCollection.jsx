...
const COLLECTION_PRICE    = 75;
const COLLECTION_SEPARATE = 89;
const COLLECTION_SAVINGS  = 14;
const BUNDLE_SATURN       = 39;
const BUNDLE_SLS          = 16;
const BUNDLE_STARSHIP     = 20;
const WEEKLY_LIMIT        = 20;
const PAGE_SOURCE         = "heavylift_landing";
...
const BENEFITS = [
  "Three rockets — Saturn V 22\", SLS 20\", Starship 10\"",
  "Snaps together in 15-20 minutes each",
  "Printed in the USA",
  "Ships in 2-4 days",
...
function SwipeCarousel({ images, accentColor, onLightbox }) {
  const [idx, setIdx] = useState(0);
  const wrapRef = useRef(null);
  const touchX  = useRef(null);
  const touchY  = useRef(null);
  ...
  const colors = {
    orange: { dot: "bg-orange-400", border: "rgba(251,146,60,0.2)",  bg: "#0c0b12" },
    blue:   { dot: "bg-blue-400",   border: "rgba(96,165,250,0.2)",  bg: "#0c0b12" },
    cyan:   { dot: "bg-cyan-400",   border: "rgba(34,211,238,0.2)",  bg: "#040d14" },
  };
  const { dot, border, bg } = colors[accentColor] ?? colors.orange;
  ...
  return (
    <div className="flex flex-col gap-2 select-none">
      <div ref={wrapRef} className="w-full rounded-2xl overflow-hidden"
        style={{ background: bg, border: `1px solid ${border}`, height: "clamp(260px, 70vw, 420px)" }}>
        <div style={{
            ...
            <div key={i} className="flex-shrink-0" style={{ minWidth: "100%", height: "100%" }}>
              {isVideo(src) ? (
                ...
                <div className="w-full h-full cursor-zoom-in" onClick={() => onLightbox(src)}>
                  <img src={src} alt={`Photo ${i + 1}`} className="w-full h-full object-contain p-3" draggable={false} />
                </div>
              )}
            </div>
          ))}
      ...
      {images.length > 1 && (
        <div className="flex justify-center gap-1.5">
          {images.map((_, i) => (
            <button key={i} onClick={() => setIdx(i)}
              className={`rounded-full transition-all duration-200 ${i === idx ? `w-4 h-1.5 ${dot}` : "w-1.5 h-1.5 bg-white/20"}`} />
          ))}
        </div>
      )}
    </div>
...
export default function HeavyLiftCollection() {
  const [adding, setAdding]     = useState(null);
  const [lightbox, setLightbox] = useState(null);
  const [showSticky, setSticky] = useState(false);
  const [starshipId, setStarshipId] = useState(null);
  const [bannerCopied, setBannerCopied] = useState(false);
  const [saturnHero, setSaturnHero]             = useState(FALLBACK_SATURN);
  const [slsHero, setSlsHero]                   = useState(FALLBACK_SLS);
  const [starshipHero, setStarshipHero]         = useState(FALLBACK_STARSHIP);
  const [saturnCarousel, setSaturnCarousel]     = useState([]);
  const [slsCarousel, setSlsCarousel]           = useState([]);
  const [starshipCarousel, setStarshipCarousel] = useState([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const saturn = await base44.entities.Product?.get(SATURN_V_ID).catch(() => null);
        const imgs = saturn?.images ?? [];
        setSaturnHero(imgs[0] ?? FALLBACK_SATURN);
        setSaturnCarousel(imgs.slice(1));
      } catch { setSaturnHero(FALLBACK_SATURN); }
      try {
        const sls = await base44.entities.Product?.get(SLS_ID).catch(() => null);
        const imgs = sls?.images ?? [];
        setSlsHero(imgs[0] ?? FALLBACK_SLS);
        setSlsCarousel(imgs.slice(1));
      } catch { setSlsHero(FALLBACK_SLS); }
      try {
        const prods    = await base44.entities.Product.filter({ status: "active" });
        const starship = prods.find(p => p.name?.toLowerCase().includes("starship"));
        if (starship) {
          setStarshipId(starship.id);
          const imgs = starship.images ?? [];
          setStarshipHero(imgs[0] ?? FALLBACK_STARSHIP);
          setStarshipCarousel(imgs.slice(1));
        } else { setStarshipHero(FALLBACK_STARSHIP); }
      } catch { setStarshipHero(FALLBACK_STARSHIP); }
    };
    load();
  ...
  const addToCart = async (type) => {
    ...
    if ((type === "starship" || type === "collection") && !starshipId) {
      toast({ title: "Still loading, please try again", variant: "destructive" });
      setAdding(null); return;
    }
    try {
      const user = await base44.auth.me().catch(() => null);
      const items = [];
      if (type === "saturn")   items.push({ product_id: SATURN_V_ID, product_name: "Saturn V",    price: SATURN_PRICE,   image: saturnHero });
      if (type === "sls")      items.push({ product_id: SLS_ID,       product_name: "SLS",         price: SLS_PRICE,      image: slsHero });
      if (type === "starship") items.push({ product_id: starshipId,   product_name: "Starship V2", price: STARSHIP_PRICE, image: starshipHero });
      if (type === "collection") {
        items.push({ product_id: SATURN_V_ID, product_name: "Saturn V (Heavy Lift Collection)",    price: BUNDLE_SATURN,   image: saturnHero });
        items.push({ product_id: SLS_ID,      product_name: "SLS (Heavy Lift Collection)",         price: BUNDLE_SLS,      image: slsHero });
        items.push({ product_id: starshipId,  product_name: "Starship V2 (Heavy Lift Collection)", price: BUNDLE_STARSHIP, image: starshipHero });
      }
      if (user) {
      ...
      if (type === "collection") {
        toast({ title: "Collection added!", description: `All 3 rockets for $${COLLECTION_PRICE}` });
  ...
  const rockets = [
    { type: "saturn",   hero: saturnHero,   carousel: saturnCarousel,   name: "Saturn V",    label: 'Saturn V 22"',  price: SATURN_PRICE,   accent: "text-orange-400", border: "rgba(251,146,60,0.3)", btn: "linear-gradient(90deg, #c2410c, #f97316)", carouselColor: "orange" },
    { type: "sls",      hero: slsHero,      carousel: slsCarousel,      name: "SLS Artemis", label: 'SLS 20"',       price: SLS_PRICE,      accent: "text-blue-400",   border: "rgba(96,165,250,0.3)",  btn: "linear-gradient(90deg, #1d4ed8, #60a5fa)", carouselColor: "blue"   },
    { type: "starship", hero: starshipHero, carousel: starshipCarousel, name: "Starship V2", label: 'Starship 10"',  price: STARSHIP_PRICE, accent: "text-cyan-400",   border: "rgba(34,211,238,0.3)",  btn: "linear-gradient(90deg, #0891b2, #38bdf8)", carouselColor: "cyan"   },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#09080e] text-white overflow-x-hidden" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 p-3 sm:hidden"
          ...
          {slotsLeft !== null && slotsLeft < WEEKLY_LIMIT && (
            <p className={`text-center text-xs font-semibold mb-2 ${urgent ? "text-red-400" : "text-amber-300"}`}>
              ...
              {slotsLeft === 0 ? "Fully booked this week" : `Only ${slotsLeft} slots left`}
            </p>
          )}
          <button onClick={() => addToCart("collection")} disabled={adding !== null}
            className="w-full py-4 rounded-full font-black text-base text-white disabled:opacity-60"
            style={{ background: "linear-gradient(90deg, #92400e, #f97316)" }}>
            {adding === "collection" ? "Adding..." : "Order Now · $75"}
          </button>
        </div>
      )}
      ...
      <section className="max-w-sm mx-auto px-5 pt-6 pb-8">
        ...
        {/* 1. HERO IMAGES — three in a row */}
        <div className="flex gap-1.5 mb-5">
          {rockets.map(({ type, hero, name, label, border }) => (
            <div key={type} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full rounded-xl overflow-hidden cursor-zoom-in"
                style={{ height: "clamp(130px, 32vw, 170px)", background: "#0c0b12", border: `1px solid ${border}` }}
                onClick={() => hero && setLightbox(hero)}>
                {hero ? (
                  <img src={hero} alt={name} className="w-full h-full object-contain p-1.5" draggable={false} />
                ) : (
                  <div className="w-full h-full flex items-center justify-center">
                    <div className="w-5 h-5 rounded-full border-2 border-orange-500/40 border-t-orange-400 animate-spin" />
                  </div>
                )}
              </div>
              <p className="text-gray-700 text-[9px]">{label}</p>
            </div>
          ))}
        </div>
        ...
        <p className="text-center text-gray-400 text-sm mb-5 leading-relaxed">
          Three 1:200 scale model kits: Saturn V at 22", SLS at 20", Starship at 10". All 3D printed, snap-fit, assemble in 15-20 minutes each. Some glue needed.
        </p>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-gray-500 line-through text-sm ml-1">${COLLECTION_SEPARATE}</span>
          </div>
          {slotsLeft !== null && slotsLeft < WEEKLY_LIMIT && (
        ...
        <div ref={heroCTARef}>
          <button onClick={() => addToCart("collection")} disabled={adding !== null}
            ...
            style={{ background: "linear-gradient(90deg, #92400e, #f97316)", boxShadow: "0 6px 28px rgba(249,115,22,0.28)" }}>
            {adding === "collection" ? "Adding..." : "Order Now"}
          </button>
        </div>
      ...
      {rockets.map(({ type, name, carousel, carouselColor, accent }) =>
        carousel.length > 0 ? (
          <section key={type} className="border-t border-white/5 py-8 px-5">
            <div className="max-w-sm mx-auto">
              <p className={`text-xs font-bold mb-3 ${accent}`}>{name}</p>
              <SwipeCarousel images={carousel} accentColor={carouselColor} onLightbox={setLightbox} />
            </div>
          </section>
        ) : null
      )}
      {rockets.some(r => r.carousel.length > 0) && (
        <p className="text-center text-gray-700 text-xs pb-2">Swipe for more photos</p>
      )}
      ...
      <section className="border-t border-white/5 py-8 px-5 bg-white/[0.02]">
        <div className="max-w-sm mx-auto space-y-5">
          {[
            ...
            { q: "Who designed these?",           a: "AstroDesign 3D (KMO Brain), one of the most respected and accurate rocket modelers in 3D printing. EX3D prints and fulfills his licensed designs." },
          ].map(({ q, a }) => (
            <div key={q}>
      ...
      {/* Individual rockets section */}
      <section className="border-t border-white/5 py-10 px-5">
        <div className="max-w-sm mx-auto">
          <p className="text-xs tracking-[0.35em] text-orange-400/70 uppercase text-center mb-4">Or add individually</p>

          <div className="flex gap-2 mb-8">
            {rockets.map(({ type, hero, name, price, accent, border, btn }) => (
              <div key={type} className="flex-1 flex flex-col items-center gap-2 bg-white/[0.03] border border-white/5 rounded-xl p-2.5">
                <div className="w-full rounded-lg overflow-hidden bg-[#0c0b12] cursor-zoom-in"
                  style={{ aspectRatio: "2/3", border: `1px solid ${border}` }}
                  onClick={() => hero && setLightbox(hero)}>
                  {hero ? (
                    <img src={hero} alt={name} className="w-full h-full object-contain p-1.5" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center">
                ...
                <p className="text-white text-[10px] font-bold text-center">{name}</p>
                <div className="flex items-center justify-between w-full">
                  <span className={`${accent} font-black text-base`}>${price}</span>
                  <button onClick={() => addToCart(type)} disabled={adding !== null}
                    className="text-[10px] px-2.5 py-1 rounded-full font-bold text-white disabled:opacity-50"
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <p className="text-gray-600 text-xs">or get the full collection</p>
            <div className="flex-1 border-t border-white/5" />
          </div>
          ...
          <div className="bg-white/[0.03] border-2 border-orange-500/30 rounded-2xl p-5 relative">
            ...
            <div className="flex items-baseline justify-center gap-3 mb-4">
              <span className="text-gray-500 line-through">${COLLECTION_SEPARATE}</span>
              <span className="font-black text-3xl text-orange-400">${COLLECTION_PRICE}</span>
            </div>
            <button onClick={() => addToCart("collection")} disabled={adding !== null}
              className="w-full py-4 rounded-full font-black text-base text-white disabled:opacity-60 transition-all hover:scale-[1.02]"
              style={{ background: "linear-gradient(90deg, #92400e, #f97316)" }}>
              {adding === "collection" ? "Adding..." : "Get The Collection"}
            </button>
