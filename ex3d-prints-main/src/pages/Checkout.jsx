...
import { Loader2, ShoppingBag, MapPin, Tag, CheckCircle, XCircle } from "lucide-react";
...
export default function Checkout() {
  ...
  const [appliedCoupon, setAppliedCoupon] = useState(null); // { code, percent_off, amount_off, valid }
  ...
  const [designerOptIn, setDesignerOptIn] = useState(false);
  ...
  const initializeCheckout = async () => {
    try {
      ...
      if (currentUser) {
        // Pre-fill address immediately so UI shows right away
        ...
        // Guest flow: load from localStorage instantly
  ...
  // Logged-in user: load cart items from DB.
  // Landing-page items already have product_name + images stored on the cart record — skip enrichment for those.
  // Only fetch product details for items that are missing print_files/weight data (e.g. items added from the Marketplace).
  const loadCartFromDB = async (userId, prefilledAddr = null) => {
    try {
      const cart = await base44.entities.Cart.filter({ user_id: userId });

      // Identify which items actually need enrichment (custom requests, or missing print data)
      const needsEnrichment = cart.filter(item => item.custom_request_id || !item.product_name);
      const alreadyComplete = cart.filter(item => !item.custom_request_id && item.product_name);

      const enrichedFromDB = await Promise.all(needsEnrichment.map(async (item) => {
        try {
          if (item.custom_request_id) {
            const customRequest = await base44.entities.CustomPrintRequest.get(item.custom_request_id);
            return { ...item, product_name: customRequest.title, print_files: customRequest.files || [], print_time_hours: customRequest.print_time_hours, weight_grams: customRequest.weight_grams, dimensions: customRequest.dimensions, images: customRequest.images || [], multi_color: false, is_custom_request: true };
          } else {
            const product = await base44.entities.Product.get(item.product_id);
            return { ...item, product_name: product.name, print_files: product.print_files || [], print_time_hours: product.print_time_hours, weight_grams: product.weight_grams, dimensions: product.dimensions, images: product.images || item.images || [], multi_color: product.multi_color, designer_id: product.designer_id, is_custom_request: false };
          }
        } catch {
          return { ...item, product_name: item.product_name || "Unknown Product", print_files: [], images: item.images || [], multi_color: false, is_custom_request: !!item.custom_request_id };
        }
      }));

      const enrichedItems = [...alreadyComplete, ...enrichedFromDB];
      setCartItems(enrichedItems);

      // Auto-calculate shipping whenever the saved address is complete.
      // calculateShippingCost falls back to a flat rate when items lack weight data,
      // so never skip it — otherwise the user is stuck on "Shipping not calculated".
      if (prefilledAddr?.street && prefilledAddr?.city && prefilledAddr?.state && prefilledAddr?.zip) {
        calculateShippingCost(prefilledAddr, enrichedItems); // fire-and-forget, don't await
      }
    } catch (error) {
  ...
  const calculateShippingCost = async (addr, items = cartItems) => {
    ...
    const hasWeightData = items.some(i => i.weight_grams);
    if (!hasWeightData) {
      // No weight data available (e.g. landing-page/guest cart items) — use flat rate
  ...
  const getShippingFee = () => shippingCost !== null ? shippingCost : 0;
  const getDiscountAmount = () => {
    if (!appliedCoupon) return 0;
    const subtotal = calculateSubtotal();
    if (appliedCoupon.percent_off) return (subtotal * appliedCoupon.percent_off) / 100;
    if (appliedCoupon.amount_off) return Math.min(appliedCoupon.amount_off / 100, subtotal);
    return 0;
  };
  const calculateTotal = () => Math.max(0, calculateSubtotal() + getShippingFee() - getDiscountAmount());
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      // Special test code — always 100% off
      if (code === 'JC3DTESTFREEDOM') {
        setAppliedCoupon({ code, percent_off: 100, amount_off: null });
        toast({ title: "Coupon applied!", description: "100% off — this order is free." });
        setApplyingCoupon(false);
        return;
      }
      // Validate against backend (which checks Stripe)
      const res = await base44.functions.invoke('validateCouponCode', { code });
      if (res.data?.valid) {
        setAppliedCoupon({ code, percent_off: res.data.percent_off || null, amount_off: res.data.amount_off || null });
        const desc = res.data.percent_off
          ? `${res.data.percent_off}% off your order`
          : `$${(res.data.amount_off / 100).toFixed(2)} off your order`;
        toast({ title: "Coupon applied!", description: desc });
      } else {
        toast({ title: "Invalid coupon", description: res.data?.error || "This code is not valid or has expired.", variant: "destructive" });
      }
    } catch {
      toast({ title: "Invalid coupon", description: "This code is not valid or has expired.", variant: "destructive" });
    }
    setApplyingCoupon(false);
  };
  ...
  const handleAddressChange = (field, value) => {
    ...
    // Recalculate shipping whenever address is complete — works for both guests and logged-in users
  ...
  const handleCheckout = async (e) => {
    ...
    if (!user && !shippingAddress.email) {
      toast({ title: "Email required", description: "Please enter your email address so we can send your order confirmation.", variant: "destructive" });
      return;
    }
    if (shippingCost === null) {
      toast({ title: "Shipping not calculated", description: "Please complete your delivery address to calculate shipping.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const invalidItems = cartItems.filter(item => {
        if (item.is_custom_request) {
          return !item.product_name || item.unit_price === undefined || item.unit_price === null || item.quantity === undefined || item.quantity === null;
        }
        return !item.product_id || !item.product_name || item.unit_price === undefined || item.unit_price === null || item.quantity === undefined || item.quantity === null;
      });
      if (invalidItems.length > 0) {
        throw new Error("Some cart items are missing required data. Please refresh your cart.");
      }

      // Fire-and-forget opt-ins — don't await, these must not block Stripe redirect
      if (!user && newsletterOptIn && shippingAddress.email) {
        base44.entities.NewsletterSubscriber.create({ email: shippingAddress.email, name: shippingAddress.name || '', source: 'checkout', opted_in: true }).catch(() => {});
      }
      if (designerOptIn && shippingAddress.email) {
        const designerIds = [...new Set(cartItems.map(i => i.designer_id).filter(Boolean))];
        designerIds.forEach(did => {
          base44.entities.DesignerFollower.create({ designer_id: did, email: shippingAddress.email, name: shippingAddress.name || '', user_id: user?.id || null, source: 'checkout', opted_in_global: newsletterOptIn }).catch(() => {});
        });
      }

      // Read affiliate ref from localStorage (set when user clicks an affiliate link)
      const affiliateRef = localStorage.getItem('affiliateRef') || undefined;

      // Grab Rewardful referral ID — with a 2s timeout so it never blocks checkout
      const rewardfulId = await Promise.race([
        typeof window.rewardful === 'function'
          ? new Promise(resolve => { try { window.rewardful('ready', ({ referral }) => resolve(referral?.id || null)); } catch { resolve(null); } })
      ...
      const checkoutData = {
        ...
        rewardfulReferralId: rewardfulId || undefined,
      ...
      if (response.data?.url) {
        ...
        // For guest users: persist checkout data so PaymentSuccess can create the order
        if (!user) {
          localStorage.setItem('pendingGuestCheckout', JSON.stringify({
            ...
            cartItems: cartItems.map(item => ({
              ...
              multi_color_selections: item.multi_color_selections || null,
            })),
          }));
        }

        // Redirect to Stripe — use window.top to break out of any iframe (e.g. preview sandbox)
        try {
          window.top.location.href = response.data.url;
        } catch {
          window.location.href = response.data.url;
        }
        return; // processing stays true — page is navigating away
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Label htmlFor="newsletter" className="text-sm text-gray-600 cursor-pointer">
                        Get updates from EX3D Prints (new releases & promotions)
                      </Label>
                    </div>
                    {cartItems.some(i => i.designer_id) && (
                      <div className="flex items-center gap-2">
                        <Checkbox
                          id="designer_follow"
                          checked={designerOptIn}
                          onCheckedChange={(checked) => setDesignerOptIn(checked)}
                        />
                        <Label htmlFor="designer_follow" className="text-sm text-gray-600 cursor-pointer">
                          Get updates from this designer when they release new products
                        </Label>
                      </div>
                    )}
                  </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div>
                  ...
                  {appliedCoupon ? (
                    <div className="flex items-center gap-2 mt-1 p-2.5 bg-green-50 border border-green-200 rounded-md">
                      ...
                      <button onClick={handleRemoveCoupon} className="text-green-700 hover:text-red-600 ml-auto flex-shrink-0">
                ...
                <div>
                  ...
                  <Input id="referral" value={referralCode} placeholder="e.g. FRIEND25"
                    onChange={(e) => setReferralCode(e.target.value)} className="text-sm" />
                  <p className="text-xs text-gray-500 mt-1">
                    If a friend referred you, enter their code here. Both of you get a reward!
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="sticky top-4">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-1.5 text-xs text-gray-600">
                  <p className="flex items-center gap-2">🔒 <span>Secure payment powered by <strong>Stripe</strong></span></p>
                  <p className="flex items-center gap-2">📦 <span>Printed in 1–3 days · ships in 3–5 business days with tracking</span></p>
                  <p className="flex items-center gap-2">📧 <span>Order confirmation + tracking sent to your email</span></p>
                </div>
