...
const BUNDLE_SAVINGS = 9;
const SEPARATE_TOTAL = 69;
const BUNDLE_SATURN  = 34;
const BUNDLE_SLS     = 26;
const WEEKLY_LIMIT   = 20;
const PAGE_SOURCE    = "moonmission_landing";
...
const BENEFITS = [
  "Two rockets: 22\" Saturn V and 20\" SLS",
  "Snaps together in 20 minutes each",
  "Printed in the USA",
  "Ships in 2-4 days",
...
export default function MoonMissions() {
  ...
  const addToCart = async (type) => {
    ...
    try {
      ...
      if (type === "sls")    items.push({ product_id: SLS_ID,       product_name: "SLS (Artemis)", price: SLS_PRICE,    image: slsHero });
      if (type === "bundle") {
        items.push({ product_id: SATURN_V_ID, product_name: "Saturn V (Moon Missions Bundle)", price: BUNDLE_SATURN, image: saturnHero });
        items.push({ product_id: SLS_ID,      product_name: "SLS (Moon Missions Bundle)",      price: BUNDLE_SLS,   image: slsHero });
      }
      if (user) {
      ...
      if (type === "bundle") {
        toast({ title: "Bundle added!", description: `Both rockets for $${BUNDLE_PRICE}` });
    ...
    <div className="min-h-screen bg-[#09080e] text-white overflow-x-hidden" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      {showSticky && (
        <div className="fixed bottom-0 left-0 right-0 z-50 p-3 sm:hidden"
          ...
          <button onClick={() => addToCart("bundle")} disabled={adding !== null}
            ...
            {adding === "bundle" ? "Adding..." : "Order Now · $60"}
      ...
      <section className="max-w-sm mx-auto px-5 pt-6 pb-8">
        ...
        {/* 1. HERO IMAGES — side by side */}
        <div className="flex gap-2 mb-5">
          {[
            { src: saturnHero, alt: "Saturn V",    label: 'Saturn V 22"',    border: "rgba(251,146,60,0.3)", glow: "rgba(251,146,60,0.08)" },
            { src: slsHero,    alt: "SLS Artemis", label: 'SLS Artemis 20"', border: "rgba(96,165,250,0.3)",  glow: "rgba(96,165,250,0.06)"  },
          ].map(({ src, alt, label, border, glow }) => (
            <div key={alt} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full rounded-xl overflow-hidden cursor-zoom-in"
                style={{ height: "clamp(160px, 40vw, 210px)", background: "#0c0b12", border: `1px solid ${border}`, boxShadow: `0 8px 32px ${glow}` }}
                onClick={() => src && setLightbox(src)}>
                {src ? (
                  <img src={src} alt={alt} className="w-full h-full object-contain p-2" draggable={false} />
                ) : (
                  <div className="w-full h-full flex items-center justify-center">
                    <div className="w-6 h-6 rounded-full border-2 border-orange-500/40 border-t-orange-400 animate-spin" />
                  </div>
                )}
              </div>
        ...
        <p className="text-center text-gray-400 text-sm mb-5 leading-relaxed">
          Two 1:200 scale model kits: Saturn V at 22" and SLS Artemis at 20". Both 3D printed, snap-fit, assemble in about 20 minutes each. Some glue needed.
        </p>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-gray-500 line-through text-sm ml-1">${SEPARATE_TOTAL}</span>
          </div>
          {slotsLeft !== null && slotsLeft < WEEKLY_LIMIT && (
        ...
        <div ref={heroCTARef}>
          <button onClick={() => addToCart("bundle")} disabled={adding !== null}
            ...
            {adding === "bundle" ? "Adding..." : "Order Now"}
          </button>
        </div>
      ...
      {saturnCarousel.length > 0 && (
        <section className="border-t border-white/5 py-8 px-5">
          <div className="max-w-sm mx-auto">
            <p className="text-xs text-orange-400 font-bold mb-3">Saturn V</p>
            <SwipeCarousel images={saturnCarousel} accentColor="orange" onLightbox={setLightbox} />
          </div>
        </section>
      )}
      {slsCarousel.length > 0 && (
        <section className="border-t border-white/5 py-8 px-5">
          <div className="max-w-sm mx-auto">
            <p className="text-xs text-blue-400 font-bold mb-3">SLS Artemis</p>
            <SwipeCarousel images={slsCarousel} accentColor="blue" onLightbox={setLightbox} />
          </div>
        </section>
      )}
      {(saturnCarousel.length > 0 || slsCarousel.length > 0) && (
      ...
      <section className="border-t border-white/5 py-10 px-5">
        <div className="max-w-sm mx-auto">
          ...
          <div className="flex gap-2 mb-8">
            {[
              { type: "saturn", src: saturnHero, label: "Saturn V",    price: SATURN_PRICE, accent: "text-orange-400", border: "rgba(251,146,60,0.3)", btn: "linear-gradient(90deg, #c2410c, #f97316)" },
              { type: "sls",    src: slsHero,    label: "SLS Artemis", price: SLS_PRICE,    accent: "text-blue-400",   border: "rgba(96,165,250,0.3)",  btn: "linear-gradient(90deg, #1d4ed8, #60a5fa)" },
            ].map(({ type, src, label, price, accent, border, btn }) => (
              <div key={type} className="flex-1 flex flex-col items-center gap-2 bg-white/[0.03] border border-white/5 rounded-xl p-3">
                ...
                <div className="flex items-center justify-between w-full">
                  <span className={`${accent} font-black text-lg`}>${price}</span>
                  <button onClick={() => addToCart(type)} disabled={adding !== null}
                    className="text-xs px-3 py-1.5 rounded-full font-bold text-white disabled:opacity-50"
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <p className="text-gray-600 text-xs">or get the bundle</p>
            <div className="flex-1 border-t border-white/5" />
          </div>
          ...
          <div className="bg-white/[0.03] border-2 border-orange-500/30 rounded-2xl p-5 relative">
            ...
            <div className="flex items-baseline justify-center gap-3 mb-4">
              <span className="text-gray-500 line-through">${SEPARATE_TOTAL}</span>
              <span className="font-black text-3xl text-orange-400">${BUNDLE_PRICE}</span>
            </div>
            <button onClick={() => addToCart("bundle")} disabled={adding !== null}
              className="w-full py-4 rounded-full font-black text-base text-white disabled:opacity-60 transition-all hover:scale-[1.02]"
              style={{ background: "linear-gradient(90deg, #c2410c, #f97316)" }}>
              {adding === "bundle" ? "Adding..." : "Get The Collection"}
            </button>
