...
import { Loader2, ShoppingCart, Trash2, Plus, Minus, ArrowRight, Package, MapPin } from "lucide-react";
...
export default function Cart() {
  ...
  const checkAuthAndLoadCart = async () => {
    ...
    try {
      ...
      // Check for saved address to show shipping estimate banner
      const addresses = currentUser.saved_addresses || [];
      const defaultAddr = addresses.find(a => a.is_default) || addresses[0];
      if (defaultAddr?.street) setSavedAddress(defaultAddr);
    } catch {
      setUser(null);
      loadAnonymousCart();
    }
  ...
  const mergeAnonymousCartIfNeeded = async (userId) => {
    ...
    if (anonWishlist.length > 0) {
      try {
        ...
        const merged = [...new Set([...currentWishlist, ...anonWishlist])];
        await base44.auth.updateMe({ wishlist: merged });
        localStorage.removeItem("anonymousWishlist");
      } catch (e) {
        console.error("Failed to merge anon wishlist:", e);
      }
    }
  };
  ...
  const loadCart = async (userId) => {
    try {
      ...
      // Fetch all product/custom-request details in parallel (was sequential — slow for multi-item carts)
      const productsData = {};
  ...
  const handleUpdateColor = async (item, newColor) => {
    try {
      if (user) {
        ...
        const updatedItems = anonymousCartGet().map(i =>
          i.id === item.id ? { ...i, selected_color: newColor } : i
        );
        anonymousCartSave(updatedItems);
  ...
  const handleProceedToCheckout = () => {
    ...
    setTimeout(() => { window.location.href = createPageUrl("Checkout"); }, 100);
  };
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {cartItems.map((item) => {
              ...
              // Resolve image from fetched product data (loaded in loadCart), fall back to item's stored images
          ...
          <div className="lg:col-span-1">
            ...
            {savedAddress && (
              <Card className="mb-3 border-teal-200 bg-teal-50">
                <CardContent className="p-3 flex items-start gap-2">
                  <MapPin className="w-4 h-4 text-teal-600 mt-0.5 flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-teal-800">Shipping to saved address</p>
                    <p className="text-xs text-teal-700 truncate">{savedAddress.street}, {savedAddress.city}, {savedAddress.state} {savedAddress.zip}</p>
                  </div>
                </CardContent>
              </Card>
            )}
            <Card className="sticky top-4">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">📦 Printed in 1–3 days · ships in 3–5 business days with USPS tracking</p>
                ...
                <div className="pt-4 border-t space-y-2">
                  <p className="text-xs text-gray-500 text-center">
                    🔒 Secure checkout powered by Stripe · 🇺🇸 Printed by vetted US makers
                  </p>
                  <p className="text-xs text-gray-500 text-center">
