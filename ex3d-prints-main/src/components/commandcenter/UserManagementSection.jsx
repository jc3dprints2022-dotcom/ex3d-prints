...
import { Search, UserCheck, UserX, Mail, Star, TrendingUp, MapPin } from "lucide-react";
import { useToast } from "@/components/ui/use-toast";

function AffiliateAddressBlock({ userId }) {
  const [address, setAddress] = React.useState(null);
  const [loaded, setLoaded] = React.useState(false);

  React.useEffect(() => {
    base44.entities.Affiliate.filter({ user_id: userId }).then(records => {
      setAddress(records[0]?.shipping_address || null);
      setLoaded(true);
  ...
  return (
    <div className="bg-slate-900 p-4 rounded-lg border border-purple-500/30">
      ...
      {address?.street ? (
        ...
        <p className="text-sm text-yellow-400">⚠️ No shipping address on file</p>
      )}
    </div>
  );
}
        ...
        <DialogContent className="bg-slate-800 border-cyan-500/30 text-white max-w-2xl">
          ...
          {selectedUser && (
            <div className="space-y-6 py-4">
              ...
              {selectedUser.business_roles?.includes('affiliate') && (
                <AffiliateAddressBlock userId={selectedUser.id} />
              )}
            </div>
          )}
          <DialogFooter>
