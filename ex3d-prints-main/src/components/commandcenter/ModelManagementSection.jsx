...
import { Loader2, Upload, X, Package, Link as LinkIcon, PlusCircle, Pencil, Crop, Home, GripVertical } from "lucide-react";
...
export default function ModelManagementSection() {
  ...
  const isVideo = (url) => url && /\.(mp4|mov|webm)(\?|$)/i.test(url);
  ...
  const handleImageUpload = async (e) => {
    ...
    const oversizedFiles = files.filter(file => {
      ...
      return file.size > (isVid ? MAX_VIDEO_SIZE : MAX_IMAGE_SIZE);
    });
    
    if (oversizedFiles.length > 0) {
      toast({
        title: "File too large",
        description: `Some files exceed size limits: ${oversizedFiles.map(f => f.name).join(', ')}`,
        variant: "destructive"
      });
    }
        ...
        <Card className="bg-slate-900 border-cyan-500/30">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <Label htmlFor="images" className="text-white">Product Images & Videos * (Multiple allowed)</Label>
                <Input
                  id="images"
                  type="file"
                ...
                />
                <p className="text-xs text-gray-500 mb-2">Images (max 20MB) or Videos — MP4, MOV, WebM (max 200MB). At least one required.</p>
                            ...
                            <Draggable key={img + idx} draggableId={`img-${idx}`} index={idx}>
                              {(dragProvided, dragSnapshot) => (
                                <div
                                  ...
                                  className={`relative rounded border ${dragSnapshot.isDragging ? 'opacity-80 ring-2 ring-cyan-400 shadow-lg' : ''}`}
                                >
                                  {/* Drag handle */}
                                  <div
                                    {...dragProvided.dragHandleProps}
                                    className="absolute top-1 right-1 z-10 bg-black/60 rounded p-0.5 cursor-grab active:cursor-grabbing"
                                  ...
                                  {isVideo(img) ? (
                                    <div className="relative">
                                      <video src={img} className="w-full h-24 object-cover rounded mb-1" muted />
                                      <span className="absolute top-1 left-1 bg-black/70 text-white text-xs px-1.5 py-0.5 rounded font-bold">VIDEO</span>
                                    </div>
                                  ) : (
                                    <img src={img} alt={`Product ${idx + 1}`} className="w-full h-24 object-cover rounded mb-1" />
                                  )}

                                  {idx === 0 && (
                                    <span className="absolute bottom-8 left-1 bg-cyan-600/90 text-white text-[10px] px-1 py-0.5 rounded font-bold">MAIN</span>
                                  )}
                                  ...
                                  <div className="flex gap-1 p-0.5">
                                    ...
                                    <Button
                                      ...
                                      variant="destructive"
                                      size="sm"
                                      className={`h-7 px-2 ${isVideo(img) ? 'w-full' : ''}`}
