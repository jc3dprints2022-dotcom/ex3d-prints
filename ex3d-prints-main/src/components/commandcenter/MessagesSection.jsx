...
function FeedbackReportsTab() {
  ...
  const openReport = async (report) => {
    ...
    try {
      const msgs = await base44.entities.MessageReply.filter({ submission_id: report.id });
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      setReplies(prev => [...prev, reply]);
      await base44.entities.FeedbackReport.update(selected.id, { status: "in_progress" });

      // Email user with link to view the conversation
      if (selected.user_email) {
        const conversationUrl = `${APP_ORIGIN}/ReportIssue?submission_id=${selected.id}`;
        await base44.functions.invoke("sendEmail", {
          to: selected.user_email,
          subject: `Re: "${selected.title}" — EX3D Prints`,
          body: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
<p>Hi ${selected.user_name || "there"},</p>
<p>We've responded to your report. Here's our reply:</p>
<div style="background:#f0fdf4;border-left:4px solid #16a34a;padding:15px;margin:15px 0;border-radius:4px;">
  <p style="margin:0;white-space:pre-wrap;">${replyText.trim().replace(/\n/g, '<br/>')}</p>
</div>
<div style="background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;padding:16px;text-align:center;margin:20px 0;">
  <p style="margin:0 0 10px;color:#0369a1;font-size:14px;font-weight:600;">View the full conversation and reply:</p>
  <a href="${conversationUrl}" style="background:#dc2626;color:white;padding:10px 24px;border-radius:6px;text-decoration:none;font-size:14px;font-weight:600;display:inline-block;">View Conversation →</a>
</div>
<div style="background:#f5f5f5;padding:15px;border-left:3px solid #ccc;margin-top:20px;">
  <p style="color:#666;font-size:12px;margin-bottom:8px;">Your original report: "${selected.title}"</p>
  <p style="color:#444;font-size:13px;">${selected.description?.replace(/\n/g, '<br/>')}</p>
</div>
<p style="color:#888;font-size:12px;margin-top:20px;">— EX3D Prints Team</p>
</div>`
        }).catch(() => {});
      }
  ...
  if (selected) {
    return (
      <div className="space-y-4">
        ...
        <Card className="bg-slate-900 border-cyan-500/20">
          <CardHeader className="pb-2"><CardTitle className="text-white text-base">Thread</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            {replies.length === 0 && <p className="text-gray-500 text-sm">No replies yet.</p>}
            {replies.map((r) => (
              <div key={r.id} className={`p-3 rounded-lg text-sm ${r.sender_type === "admin" ? "bg-teal-900/40 border border-teal-700/30 ml-8" : "bg-slate-800 border border-slate-700"}`}>
                <div className="flex justify-between mb-1">
                  <span className={`font-semibold ${r.sender_type === "admin" ? "text-teal-400" : "text-gray-300"}`}>
                    {r.sender_type === "admin" ? "You (Admin)" : selected.user_name || "User"}
                  </span>
                  <span className="text-gray-500 text-xs">{new Date(r.created_date).toLocaleString()}</span>
                </div>
                <p className="text-gray-200 whitespace-pre-wrap">{r.content}</p>
            ...
            <Textarea value={replyText} onChange={(e) => setReplyText(e.target.value)}
              placeholder="Write a reply (will be emailed to the submitter)..." rows={3}
              className="bg-slate-800 border-slate-600 text-white placeholder-gray-500" />
            <Button onClick={sendReply} disabled={sendingReply || !replyText.trim()} className="bg-teal-600 hover:bg-teal-700 text-white">
              {sendingReply ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Send className="w-4 h-4 mr-2" />}
              Send Reply
