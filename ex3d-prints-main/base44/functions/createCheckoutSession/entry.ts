...
Deno.serve(async (req) => {
    try {
        ...
        const {
            ...
            rewardfulReferralId, // passed by Checkout.jsx from the Rewardful snippet
            affiliateRef, // affiliate ID from localStorage when user clicked an affiliate link
        } = body;
        ...
        const sessionData = {
            ...
            // Rewardful requires client_reference_id to attribute affiliate conversions
            ...(rewardfulReferralId ? { client_reference_id: rewardfulReferralId } : {}),
            metadata: {
                ...
                // Rewardful affiliate referral ID — used by Rewardful to attribute conversions
            rewardful_referral_id: rewardfulReferralId || '',
                // Internal affiliate tracking
            ...
            // Serialize cart items so guest orders can be recovered in verifyPaymentAndCreateOrder
                ...
                // IMPORTANT: Stripe caps metadata values at 500 chars. Never truncate the JSON
                // string itself — that produces invalid JSON that breaks order recovery.
                // Instead, progressively shrink the payload and omit it entirely if it can't fit.
                items_json: (() => {
                    ...
                    let json = JSON.stringify(fullItems);
                    if (json.length <= 500) return json;
                    // Compact fallback: just enough to re-enrich items server-side
                    ...
                    json = JSON.stringify(compactItems);
                    if (json.length <= 500) return json;
                    // Minimal fallback: IDs and quantities only
                    ...
                    json = JSON.stringify(minimalItems);
                    if (json.length <= 500) return json;
                    // Can't fit valid JSON — omit rather than store a corrupt value.
                    console.warn('items_json exceeds 500 chars even minimized — omitting from metadata');
                    return '';
                })(),
            },
        };
