...
Deno.serve(async (req) => {
  try {
    ...
    // Also track active orders per printer for the 1-order-per-printer rule
    const activeOrdersMap = {};
    const queueHoursMap = {};
    const activePrinterOrdersMap = {}; // printerCount occupied per maker
    allOrders.forEach(o => {
      ...
      // Each in-progress order occupies one printer slot
    ...
    for (const maker of enrichedUsers) {
      ...
      // 1-order-per-printer rule: skip if all printers are occupied
      const occupiedPrinters = activePrinterOrdersMap[maker.maker_id] || 0;
      if (occupiedPrinters >= makerPrinters.length) {
        console.log(`Skipping maker ${maker.maker_id}: all ${makerPrinters.length} printer(s) busy (${occupiedPrinters} active orders)`);
        continue;
      }
    ...
    rankedMakers.sort((a, b) => {
      // Always prefer jc3dprints2022 first if eligible
      const aIsPriority = (a.maker.email || '').toLowerCase().includes('jc3dprints2022');
      const bIsPriority = (b.maker.email || '').toLowerCase().includes('jc3dprints2022');
