...
Deno.serve(async (req) => {
  try {
    ...
    // Fetch all users once — used for both designer and maker lookups
    ...
    for (const [designerId, earnings] of Object.entries(byDesigner)) {
      ...
      try {
        // Find designer user — support both id and designer_id lookups
        const designerUser = allUsers.find(u => u.designer_id === designerId || u.id === designerId);
        const designerAccountId = designerUser?.stripe_connect_account_id || designerUser?.stripe_account_id;
        if (!designerAccountId) {
          throw new Error(`Designer ${designerId} has no connected Stripe account`);
        }
        if (!designerUser?.stripe_connect_onboarding_complete) {
          throw new Error(`Designer ${designerId} has not completed Stripe onboarding`);
        }
...
<p>Thank you,<br/>The EX3D Team</p>
    ...
    // Group by maker_user_id (preferred) falling back to maker_id
    const byMaker = {};
    for (const e of pendingMakerEarnings) {
      const key = e.maker_user_id || e.maker_id;
      if (!key) continue; // skip records with no identifiable maker
    ...
    for (const [makerKey, earnings] of Object.entries(byMaker)) {
      ...
      try {
        ...
        const makerAccountId = makerUser?.stripe_connect_account_id || makerUser?.stripe_account_id;
        if (!makerAccountId) {
          throw new Error(`Maker ${makerKey} has no connected Stripe account`);
        }
        if (!makerUser?.stripe_connect_onboarding_complete) {
          throw new Error(`Maker ${makerKey} has not completed Stripe onboarding`);
        }
...
<p>Thank you,<br/>The EX3D Team</p>
    ...
    const pendingAffiliateEarnings = await base44.asServiceRole.entities.AffiliateEarnings.filter({ status: 'pending' }).catch(() => []);

    // Group by affiliate_id
    const byAffiliate = {};
    for (const e of pendingAffiliateEarnings) {
      if (!e.affiliate_id) continue;
      if (!byAffiliate[e.affiliate_id]) byAffiliate[e.affiliate_id] = [];
      byAffiliate[e.affiliate_id].push(e);
    }

    const allAffiliates = await base44.asServiceRole.entities.Affiliate.list().catch(() => []);

    for (const [affiliateId, earnings] of Object.entries(byAffiliate)) {
      const totalCents = Math.round(earnings.reduce((s, e) => s + e.commission_amount, 0) * 100);
      if (totalCents < 100) continue;

      try {
        const affiliateRecord = allAffiliates.find(a => a.affiliate_id === affiliateId);
        const affiliateUser = affiliateRecord ? allUsers.find(u => u.id === affiliateRecord.affiliate_user_id) : null;
        const affiliateAccountId = affiliateUser?.stripe_connect_account_id || affiliateUser?.stripe_account_id;

        if (!affiliateAccountId) throw new Error(`Affiliate ${affiliateId} has no connected Stripe account`);
        if (!affiliateUser?.stripe_connect_onboarding_complete) throw new Error(`Affiliate ${affiliateId} has not completed Stripe onboarding`);

        const transfer = await stripe.transfers.create({
          amount: totalCents,
          currency: 'usd',
          destination: affiliateAccountId,
          description: `Affiliate commission payout — ${new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}`,
        });

        const paidDate = new Date().toISOString();
        for (const e of earnings) {
          await base44.asServiceRole.entities.AffiliateEarnings.update(e.id, {
            status: 'paid',
            paid_date: paidDate,
            stripe_transfer_id: transfer.id,
          });
        ...
        if (affiliateRecord) {
          const newTotal = (affiliateRecord.total_earnings || 0) + totalCents / 100;
          await base44.asServiceRole.entities.Affiliate.update(affiliateRecord.id, { total_earnings: newTotal });
        }

        await base44.asServiceRole.integrations.Core.SendEmail({
          to: affiliateUser.email,
          subject: `💸 Affiliate Commission Paid — $${(totalCents / 100).toFixed(2)}`,
          body: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
<h2 style="color:#0d9488;">Your Affiliate Commission Has Been Sent!</h2>
<p>Hi ${affiliateUser.full_name},</p>
<p>We've just transferred <strong>$${(totalCents / 100).toFixed(2)}</strong> in affiliate commissions to your Stripe account for <strong>${earnings.length} referred order${earnings.length > 1 ? 's' : ''}</strong> this period.</p>
<p>Funds typically arrive in 2–3 business days.</p>
<p>Thank you for promoting EX3D Prints!<br/>The EX3D Team</p>
</div>`,
        }).catch(() => {});
      } catch (err) {
        errors.push({ type: 'affiliate', id: affiliateId, error: err.message });
        for (const e of earnings) {
          await base44.asServiceRole.entities.AffiliateEarnings.update(e.id, { status: 'transfer_failed' }).catch(() => {});
        }
        await base44.asServiceRole.integrations.Core.SendEmail({
          to: ADMIN_EMAIL,
          subject: `⚠️ Affiliate Commission Transfer Failed — ${affiliateId}`,
          body: `<p>Affiliate ID: ${affiliateId}</p><p>Amount: $${(totalCents / 100).toFixed(2)}</p><p>Error: ${err.message}</p>`,
        }).catch(() => {});
      }
    }

    return Response.json({ success: true, designersPaid, makersPaid, affiliatesPaid, errors });
  } catch (error) {
