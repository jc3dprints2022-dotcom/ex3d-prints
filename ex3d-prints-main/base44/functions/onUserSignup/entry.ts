...
Deno.serve(async (req) => {
    ...
    try {
        ...
        }).catch(e => console.warn('Could not log EXP transaction:', e.message));

        const APP_URL = 'https://ex3dprints.com';
        const emailBody = `
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;background:#ffffff;">
    ...
    <div style="background:linear-gradient(135deg,#14b8a6 0%,#0d9488 100%);border-radius:12px;padding:30px;text-align:center;margin-bottom:30px;">
        ...
        <a href="${APP_URL}/ConsumerDashboard?tab=exp" style="display:inline-block;background:white;color:#14b8a6;padding:14px 32px;text-decoration:none;border-radius:8px;font-weight:bold;font-size:16px;">View My EXP →</a>
    </div>
    ...
    <div style="text-align:center;margin:30px 0;">
        <a href="${APP_URL}/Marketplace" style="display:inline-block;background:#14b8a6;color:white;padding:14px 32px;text-decoration:none;border-radius:8px;font-weight:bold;font-size:16px;">Start Shopping Now</a>
    </div>
    <div style="border-top:1px solid #e5e7eb;padding-top:20px;margin-top:30px;">
        <p style="color:#9ca3af;font-size:14px;text-align:center;margin:0;">
            The Best Rocket Models You Can Buy · <a href="mailto:jc3dprints2022@gmail.com" style="color:#14b8a6;">jc3dprints2022@gmail.com</a>
        </p>
    </div>
</div>`.trim();
        ...
        return Response.json({ success: true, exp_awarded: 120 });

    } catch (error) {
        console.error('❌ User signup handler error:', error.message);
