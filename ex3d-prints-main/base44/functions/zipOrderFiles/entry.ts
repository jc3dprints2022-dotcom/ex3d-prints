...
${order.shipping_address?.street
  ? `Ship To: ${order.shipping_address.name || order.customer_id}, ${order.shipping_address.street}, ${order.shipping_address.city}, ${order.shipping_address.state} ${order.shipping_address.zip}`
  : 'Delivery: local drop-off — coordinate with the customer'}

Print the files in this folder and prepare them for ${order.shipping_address?.street ? 'shipping (a label is generated when you mark the order done printing)' : 'local drop-off'}.
`;
