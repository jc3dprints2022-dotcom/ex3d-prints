...
Deno.serve(async (req) => {
  try {
    ...
    // Support both direct calls ({ orderId }) and entity automation payloads ({ event, data })
    const orderId = body.orderId || body.event?.entity_id || body.data?.id;
    ...
    const emailBody = `
      ...
      <p><a href="${Deno.env.get('BASE44_APP_URL') || 'https://ex3dprints.com'}/ConsumerDashboard?tab=orders&orderId=${orderId}&review=true" style="background: #14b8a6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">Leave a Review</a></p>
