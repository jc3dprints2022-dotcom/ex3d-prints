...
export default function SaturnV() {
  ...
  return (
    <div className="min-h-screen bg-[#080810] text-white overflow-x-hidden" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <section className="relative flex flex-col items-center justify-center px-5 text-center overflow-hidden pt-10 pb-16">
        ...
        <div className="relative z-10 max-w-5xl mx-auto w-full">
          ...
          <p className="text-base sm:text-lg text-gray-400 mb-2 max-w-xl mx-auto leading-relaxed">
            Precision-printed Saturn V, SLS &amp; Starship V2 model kits. Designed by aerospace nerds, printed by a local maker, shipped in days.
          </p>
      ...
      {/* ── SOCIAL PROOF BAR ── */}
      {/* TODO: replace with real reviews once you have them */}
      <section className="py-6 px-5 border-y border-white/5 bg-white/[0.02]">
        <div className="max-w-3xl mx-auto flex flex-wrap justify-center gap-6 text-center">
          {[
            { stat: `${MAKER_COUNT}`,    sub: "vetted makers"       },
            { stat: `${MAKER_STATES}`,   sub: "states covered"      },
            { stat: "2–4 days",          sub: "average ship time"   },
      ...
      <section className="py-16 px-5 border-t border-white/5">
        <div className="max-w-3xl mx-auto">
          <p className="text-[10px] tracking-[0.45em] text-orange-400 uppercase text-center mb-3">Best Value</p>
          <h2 className="text-3xl sm:text-4xl font-black text-center mb-2">The Heavy-Lift Bundle</h2>
          <p className="text-gray-400 text-center mb-8 max-w-lg mx-auto">All three of the most powerful rockets ever built, at 1:200 scale. The complete collection, shipped in one box.</p>

          {/* Bundle image strip */}
          <div className="flex gap-3 mb-8 h-52 rounded-2xl overflow-hidden" style={{ border: "1.5px solid rgba(251,146,60,0.4)", boxShadow: "0 0 60px rgba(251,146,60,0.08)" }}>
          ...
          <div className="grid grid-cols-3 gap-3 mb-8 text-center">
            {[
              { name: "Saturn V", spec: "56cm · 1:200", price: `$${SATURN_V_PRICE}`, color: "text-orange-400" },
              { name: "SLS",      spec: "50cm · 1:200", price: `$${SLS_PRICE}`,      color: "text-blue-400"   },
              { name: "Starship V2", spec: "26cm · 1:200", price: `$${STARSHIP_PRICE}`, color: "text-cyan-400" },
            ].map(({ name, spec, price, color }) => (
              <div key={name} className="bg-white/3 border border-white/5 rounded-xl py-3 px-2">
                <p className={`font-bold text-sm ${color}`}>{name}</p>
                <p className="text-gray-600 text-xs mt-0.5">{spec}</p>
                <p className="text-gray-400 text-xs mt-1">{price} separately</p>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col items-center gap-3">
            <div className="flex items-baseline gap-3 flex-wrap justify-center">
              <span className="text-gray-600 line-through text-lg">${GIANTS_BUNDLE_SEPARATE}</span>
              <span className="text-5xl font-black text-white">${GIANTS_BUNDLE_PRICE}</span>
              <span className="text-sm font-bold text-orange-400 bg-orange-500/10 px-3 py-1 rounded-full">Save ${GIANTS_BUNDLE_SAVINGS}</span>
            </div>
            <BundleBtn size="lg" />
            <p className="text-xs text-gray-600">Ships in {SHIPPING_DAYS} · Printed near you · Free replacement parts if anything's wrong</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-5 border-t border-white/5">
        <div className="max-w-3xl mx-auto">
          <p className="text-xs text-gray-600 text-center mb-6">Prefer just one? Get any rocket individually.</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={type} className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex flex-col">
                ...
                <SingleBtn type={type}>Add to Cart — ${price}</SingleBtn>
              </div>
            ))}
          </div>
          {/* Nudge back to bundle */}
          <div className="mt-6 text-center">
            <p className="text-xs text-gray-600 mb-2">Get all three and save ${GIANTS_BUNDLE_SAVINGS}</p>
            <BundleBtn size="base" />
          </div>
        </div>
      </section>
